...
exports.registrar = async (req, res) => {
  try {
    ...
    console.log('FILE:', req.file ? 'SIM - ' + req.file.size + ' bytes' : 'NAO');
    ...
    if (req.file) {
      ...
      const result = await new Promise((resolve, reject) => {
        cloudinary.uploader.upload_stream({ folder: 'pontos' }, (err, r) => {
          if (err) { console.log('Erro Cloudinary:', err); reject(err); }
          else { console.log('Cloudinary OK:', r.secure_url); resolve(r); }
        }).end(req.file.buffer);
      });
  ...
  } catch (e) { console.log('ERRO:', e.message); res.status(500).json({ erro: e.message }); }
};
